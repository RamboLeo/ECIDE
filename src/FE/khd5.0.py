...
class PythonIDEClient:
    ...
    def create_widgets(self):
        ...
        # 管理员面板按钮（仅管理员可见）
        self.admin_panel_btn = ttk.Button(toolbar_frame, text="管理员面板", command=self.show_admin_panel)
        self.admin_panel_btn.pack(side=tk.RIGHT, padx=(5, 0))
        self.admin_panel_btn.pack_forget()  # 默认隐藏
    ...
    def show_admin_panel(self):
        ...
        if not self.admin_token:
            messagebox.showwarning("警告", "请先以管理员身份登录")
            return
        ...
        dialog_width = int(parent_width * 0.75)
        dialog_height = int(parent_height * 0.75)

        # 居中显示
        x = self.root.winfo_rootx() + (parent_width - dialog_width) // 2
        y = self.root.winfo_rooty() + (parent_height - dialog_height) // 2
        ...
        # 用户管理工具栏
        user_toolbar = ttk.Frame(user_frame)
        user_toolbar.pack(fill=tk.X, padx=5, pady=5)

        ttk.Button(user_toolbar, text="刷新用户列表", command=lambda: self.refresh_user_list(user_tree)).pack(
            side=tk.LEFT)
        ttk.Button(user_toolbar, text="添加用户", command=lambda: self.add_user_dialog(user_tree)).pack(side=tk.LEFT,
                                                                                                        padx=(5, 0))
        ttk.Button(user_toolbar, text="修改用户", command=lambda: self.edit_user_dialog(user_tree)).pack(side=tk.LEFT,
                                                                                                         padx=(5, 0))
        ttk.Button(user_toolbar, text="删除用户", command=lambda: self.delete_user(user_tree)).pack(side=tk.LEFT,
                                                                                                    padx=(5, 0))
        ...
        # 创建带滚动条的Treeview
        user_columns = ("ID", "用户名", "注册时间", "最后登录", "是否管理员")
        user_tree = ttk.Treeview(user_list_frame, columns=user_columns, show="headings")
        ...
        user_tree.configure(xscrollcommand=hsb.set)
        ...
        # 项目下拉工具栏
        project_toolbar = ttk.Frame(project_frame)
        project_toolbar.pack(fill=tk.X, padx=5, pady=5)

        ttk.Label(project_toolbar, text="选择项目:").pack(side=tk.LEFT)
        project_combo = ttk.Combobox(project_toolbar, state="readonly", width=30)
        project_combo.pack(side=tk.LEFT, padx=(5, 0))

        ttk.Button(project_toolbar, text="刷新项目列表", command=lambda: self.refresh_project_list(project_combo)).pack(
            side=tk.LEFT, padx=(5, 0))
        ttk.Button(project_toolbar, text="拉取项目", command=lambda: self.pull_project(project_combo)).pack(
            side=tk.LEFT, padx=(5, 0))

        # 项目详情区域
        project_info_frame = ttk.LabelFrame(project_frame, text="项目详情")
        project_info_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        project_info_text = scrolledtext.ScrolledText(project_info_frame, height=15, font=("Consolas", 10))
        project_info_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        project_info_text.insert(tk.END, "选择项目后显示详情信息...")

        # 初始化用户列表
        self.refresh_user_list(user_tree)

        # 初始化项目列表
        self.refresh_project_list(project_combo)

    def refresh_user_list(self, user_tree):
        """刷新用户列表"""
        try:
    ...
    def add_user_dialog(self, user_tree):
        """添加用户对话框"""
        dialog = tk.Toplevel(self.root)
        dialog.title("添加用户")
        ...
        ttk.Checkbutton(dialog, text="管理员权限", variable=is_admin_var).pack(pady=10)
        ...
        def add_user():
            ...
            try:
                ...
                response = requests.post(f"{self.server_url}/admin/users", json=data, headers=headers)
                ...
                if result.get("success"):
                    ...
                    messagebox.showerror("错误", result.get("message", "添加用户失败"))
            except Exception as e:
                messagebox.showerror("错误", f"连接服务器失败: {str(e)}")
    ...
    def edit_user_dialog(self, user_tree):
        """修改用户对话框"""
        ...
        dialog = tk.Toplevel(self.root)
        dialog.title(f"修改用户 - {username}")
        dialog.geometry("300x300")
        dialog.transient(self.root)
        dialog.grab_set()

        ttk.Label(dialog, text="新用户名:").pack(pady=10)
        username_entry = ttk.Entry(dialog, width=30)
        ...
        is_admin_var = tk.BooleanVar(value=user_data[4] == "是")
        ttk.Checkbutton(dialog, text="管理员权限", variable=is_admin_var).pack(pady=10)
        ...
        def update_user():
            ...
            if not new_username:
                ...
                try:
                    ...
                    response = requests.put(f"{self.server_url}/admin/users/{user_id}", json=data, headers=headers)
                    result = response.json()
        ...
        ttk.Button(dialog, text="更新", command=update_user).pack(pady=10)
        username_entry.focus()

    def delete_user(self, user_tree):
        """删除用户"""
        selection = user_tree.selection()
        if not selection:
            messagebox.showwarning("警告", "请先选择一个用户")
    ...
    def refresh_project_list(self, project_combo):
        """刷新项目列表（预留功能）"""
        # 这里只是模拟数据，实际应该从服务器获取项目列表
        projects = ["项目A", "项目B", "项目C"]
        project_combo['values'] = projects
        if projects:
            project_combo.set(projects[0])

    def pull_project(self, project_combo):
        """拉取项目（预留功能）"""
        selected_project = project_combo.get()
        if selected_project:
            messagebox.showinfo("提示", f"准备拉取项目: {selected_project}\n此功能待实现")
