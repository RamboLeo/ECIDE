...
class PythonIDEClient:
    ...
    def show_admin_login_dialog(self):
        ...
        # 登录表单
        login_frame = ttk.LabelFrame(main_frame, text="管理员登录", padding=20)
        login_frame.pack(fill=tk.BOTH, expand=True)

        ttk.Label(login_frame, text="管理员用户名:").pack(pady=10)
        username_entry = ttk.Entry(login_frame, width=30)
        username_entry.pack(pady=5)

        ttk.Label(login_frame, text="密码:").pack(pady=(10, 0))
            ...
            try:
                ...
                if data.get("success"):
                    ...
                    if not user_info.get("is_admin"):
                        messagebox.showerror("错误", "该账号没有管理员权限", parent=dialog)
                        return

                    self.admin_token = data.get("token")
                    self.current_admin = user_info
                    self.admin_status_label.config(text=f"管理员: {username}", foreground="green")
                    self.admin_login_btn.config(text="管理面板", command=self.show_admin_panel)

                    dialog.destroy()
                    self.show_admin_panel()
                else:
                    messagebox.showerror("错误", data.get("message", "登录失败"), parent=dialog)
            except Exception as e:
                messagebox.showerror("错误", f"连接服务器失败: {str(e)}", parent=dialog)

        button_frame = ttk.Frame(login_frame)
        button_frame.pack(pady=20)

        ttk.Button(button_frame, text="登录", command=login).pack(side=tk.LEFT, padx=10)
    ...
    def show_admin_panel(self):
        ...
        ttk.Button(toolbar, text="刷新", command=lambda: self.load_users(user_tree)).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(toolbar, text="添加用户", command=lambda: self.show_add_user_dialog(user_tree)).pack(side=tk.LEFT,
                                                                                                        padx=5)
        ttk.Button(toolbar, text="编辑用户", command=lambda: self.show_edit_user_dialog(user_tree)).pack(side=tk.LEFT,
                                                                                                         padx=5)
        ttk.Button(toolbar, text="删除用户", command=lambda: self.delete_user(user_tree)).pack(side=tk.LEFT, padx=5)
        ttk.Button(toolbar, text="设为管理员", command=lambda: self.toggle_admin(user_tree, True)).pack(side=tk.LEFT,
                                                                                                        padx=5)
        ttk.Button(toolbar, text="取消管理员", command=lambda: self.toggle_admin(user_tree, False)).pack(side=tk.LEFT,
                                                                                                         padx=5)

        # 用户列表
        columns = ("ID", "用户名", "管理员", "注册时间", "最后登录")
        user_tree = ttk.Treeview(user_frame, columns=columns, show="headings", selectmode="browse")

        # 设置列宽
        user_tree.column("ID", width=50, anchor="center")
        user_tree.column("用户名", width=150)
        user_tree.column("管理员", width=80, anchor="center")
        user_tree.column("注册时间", width=200)
        user_tree.column("最后登录", width=200)
        ...
        # 加载用户数据
        self.load_users(user_tree)

    def load_users(self, tree):
        """加载用户列表"""
        try:
            headers = {"Authorization": f"Bearer {self.admin_token}"}
            response = requests.get(f"{self.server_url}/users", headers=headers)
            data = response.json()
    ...
    def show_add_user_dialog(self, tree):
        """显示添加用户对话框"""
        dialog = tk.Toplevel(self.root)
        dialog.title("添加用户")
        ...
        ttk.Checkbutton(dialog, text="设为管理员", variable=is_admin).pack(pady=10)
        ...
        def add_user():
            ...
            try:
                ...
                response = requests.post(f"{self.server_url}/users", json=data, headers=headers)
                ...
                if result.get("success"):
                    ...
                    messagebox.showerror("错误", result.get("message", "添加用户失败"), parent=dialog)
            except Exception as e:
                messagebox.showerror("错误", f"添加用户失败: {str(e)}", parent=dialog)

        button_frame = ttk.Frame(dialog)
        button_frame.pack(pady=20)

        ttk.Button(button_frame, text="添加", command=add_user).pack(side=tk.LEFT, padx=10)
        ttk.Button(button_frame, text="取消", command=dialog.destroy).pack(side=tk.LEFT)

        username_entry.focus()

    def show_edit_user_dialog(self, tree):
        """显示编辑用户对话框"""
        ...
        username = tree.item(selection[0], "values")[1]
        ...
        ttk.Label(dialog, text="新密码 (留空则不修改):").pack(pady=(10, 0))
        password_entry = ttk.Entry(dialog, width=30, show="*")
        password_entry.pack(pady=5)
        ...
        is_admin = tk.BooleanVar()
        is_admin.set(tree.item(selection[0], "values")[2] == "是")
        ttk.Checkbutton(dialog, text="设为管理员", variable=is_admin).pack(pady=10)
        ...
        def update_user():
            ...
            try:
                ...
                response = requests.put(f"{self.server_url}/users/{user_id}", json=data, headers=headers)
                result = response.json()

                if result.get("success"):
                    messagebox.showinfo("成功", "用户信息更新成功", parent=dialog)
                    dialog.destroy()
                    self.load_users(tree)
                else:
                    messagebox.showerror("错误", result.get("message", "更新用户失败"), parent=dialog)
            except Exception as e:
                messagebox.showerror("错误", f"更新用户失败: {str(e)}", parent=dialog)

        button_frame = ttk.Frame(dialog)
        button_frame.pack(pady=20)

        ttk.Button(button_frame, text="更新", command=update_user).pack(side=tk.LEFT, padx=10)
        ttk.Button(button_frame, text="取消", command=dialog.destroy).pack(side=tk.LEFT)
    ...
    def delete_user(self, tree):
        ...
        if messagebox.askyesno("确认删除", f"确定要删除用户 {username} 吗？"):
            try:
                headers = {"Authorization": f"Bearer {self.admin_token}"}
                response = requests.delete(f"{self.server_url}/users/{user_id}", headers=headers)
                result = response.json()
    ...
    def toggle_admin(self, tree, make_admin):
        """切换用户管理员状态"""
        selection = tree.selection()
        if not selection:
            messagebox.showwarning("警告", "请先选择一个用户")
        ...
        user_id = tree.item(selection[0], "values")[0]
        username = tree.item(selection[0], "values")[1]
        current_status = tree.item(selection[0], "values")[2] == "是"

        if make_admin and current_status:
            messagebox.showinfo("提示", "用户已经是管理员")
            return
        elif not make_admin and not current_status:
            messagebox.showinfo("提示", "用户不是管理员")
            return

        action = "设为管理员" if make_admin else "取消管理员"
        if messagebox.askyesno("确认", f"确定要将用户 {username} {action} 吗？"):
            try:
                headers = {"Authorization": f"Bearer {self.admin_token}"}
                data = {"is_admin": make_admin}

                response = requests.put(f"{self.server_url}/users/{user_id}/admin", json=data, headers=headers)
                result = response.json()
                ...
                if result.get("success"):
                    ...
                    messagebox.showerror("错误", result.get("message", f"{action}操作失败"))
            except Exception as e:
                messagebox.showerror("错误", f"{action}操作失败: {str(e)}")
        ...
        def register():
            ...
            try:
                ...
                if data.get("success"):
                    ...
                    messagebox.showerror("错误", data.get("message", "注册失败"), parent=dialog)
            except Exception as e:
                messagebox.showerror("错误", f"连接服务器失败: {str(e)}", parent=dialog)
        ...
        def login():
            ...
            try:
                ...
                if data.get("success"):
                    ...
                    messagebox.showinfo("成功", "登录成功!", parent=dialog)
                    dialog.destroy()
                else:
                    messagebox.showerror("错误", data.get("message", "登录失败"), parent=dialog)
            except Exception as e:
